...
### 프롬프트 최적화 후 테스트 결과 (2026-05-09)

Reranker 도입(2026-05-07) 이후 추가된 변경사항 — Clarify 단계 재설계 + 프롬프트 최적화 — 에 대한 효과를 동일한 3개 질문으로 측정했습니다.

#### 측정 항목 정의

| 항목 | 설명 |
|---|---|
| Clarify 분류 정확도 | LLM이 질문 유형을 올바르게 context / direct / options 로 분류하는가 |
| 인라인 인용 수 | 답변 내 `[1]`, `[2]` 형식 출처 표기 횟수 |
| 핵심 청크 순위 유지 | 2026-05-07 기준 최적 순위가 유지되는가 |
| Graceful refusal | 문서에 없는 정보를 막연히 거절하지 않고 구체적으로 설명하는가 |
...
| 항목 | 2026-05-07 | 2026-05-09 (현재) |
|---|---|---|
| Clarify 기능 | 없음 (문서 카드 선택) | **context 분류 정확** ✅ |
| AI 재질의 | 없음 | "몇 학년도 입학생이신가요?" |
| 분류 소요시간 | — | 11,812ms |

**Answer 단계**

| 항목 | 2026-05-07 (Reranker 후) | 2026-05-09 (현재) |
|---|---|---|
| latency_ms | 1,652ms (검색만) | 17,288ms (검색+LLM) |
| 인라인 인용 수 | **0개** | **35개** ✅ |
| 인용 종류 | — | [1][2][4][5] |
| sources 수 | 5개 | 5개 |
| 핵심 청크 (p.192) | 2위 | 포함 ✅ |
| Graceful refusal | 없음 | "총 취득 학점은 현재 제공된 문서만으로는 확인할 수 없습니다" ✅ |

> 가장 큰 변화: 인라인 인용 **0 → 35개**. 답변 내 모든 사실에 `[1]`, `[2]` 출처 번호가 붙어 추적 가능해짐.  
> 또한 맥락 부족(학년도 미명시)을 정확히 감지해 재질의 — 이전에는 오답 또는 "찾을 수 없음" 반환.
...
| 항목 | 2026-05-07 | 2026-05-09 (현재) |
|---|---|---|
| Clarify 기능 | 없음 | **direct 분류 정확** ✅ (사실 기반 단순 질문) |
| Clarify 스킵 | — | 즉시 답변 단계로 |

**Answer 단계**

| 항목 | 2026-05-07 (Reranker 후) | 2026-05-09 (현재) |
|---|---|---|
| latency_ms | 1,410ms (검색만) | 12,230ms (검색+LLM) |
| 인라인 인용 수 | **0개** | **5개** ✅ |
| p.252 (23. 매출액) 순위 | **1위** ✅ | **1위 유지** ✅ |
| sources 수 | 5개 | 5개 |
| Graceful refusal | 없음 | "2026년 매출액 수치를 문서에서 확인할 수 없습니다. [1][2][3][4][5] 각 청크별 이유 설명" ✅ |

> Reranker가 끌어올린 p.252 순위(**1위**)가 프롬프트 최적화 이후에도 유지됨.  
> Graceful refusal: 이전에는 막연한 "찾지 못했습니다" → 현재는 각 청크별로 왜 2026년 매출액이 없는지 구체 설명.
...
| 항목 | 2026-05-07 | 2026-05-09 (현재) |
|---|---|---|
| Clarify 기능 | 없음 | **context 분류 정확** ✅ |
| AI 재질의 | 없음 | "어떤 학과/전공의 몇 학번이신가요?" |

**Answer 단계**

| 항목 | 2026-05-07 (Reranker 후) | 2026-05-09 (현재) |
|---|---|---|
| latency_ms | 1,276ms (검색만) | 14,439ms (검색+LLM) |
| 인라인 인용 수 | **0개** | **8개** ✅ |
| p.2 (개정 적용원칙) 순위 | **1위** ✅ | **1위 유지** ✅ |
| 2024년 문서 혼입 | 제거됨 (Reranker) | [5]로 포함, 출처 명시됨 |
| Graceful refusal | 없음 | 확인된 내용과 "상세 내용은 확인 불가" 구분 ✅ |

---

#### 종합 비교

| 측정 항목 | 2026-05-07 (Reranker 도입) | 2026-05-09 (프롬프트 최적화) | 변화 |
|---|---|---|---|
| 인라인 인용 (Q1) | 0개 | **35개** | +35 |
| 인라인 인용 (Q2) | 0개 | **5개** | +5 |
| 인라인 인용 (Q3) | 0개 | **8개** | +8 |
| Clarify 분류 정확도 | 기능 없음 | **3/3 정확** (100%) | 신규 |
| 핵심 청크 1위 유지 (Q2 p.252) | 1위 | **1위 유지** | 유지 |
| 핵심 청크 1위 유지 (Q3 p.2) | 1위 | **1위 유지** | 유지 |
| Graceful refusal | 없음 | **3/3 구체 설명** | 신규 |
| 검색 응답 지연 (Reranker 기여) | +700~1400ms | +700~1400ms | 동일 |
| 전체 응답 지연 (LLM 포함) | 미측정 | 12,230~17,288ms | 측정 완료 |

**두 개선의 역할 분리**

| 개선 | 시점 | 무엇을 바꾸는가 | 측정 방법 |
|---|---|---|---|
| Reranker 도입 | 2026-05-07 | **어떤 청크를 가져오는가** — 검색 순위 정밀화 | 청크 순위, 중복/타 학과 혼입 여부 |
| 프롬프트 최적화 | 2026-05-09 | **가져온 청크로 무엇을 만드는가** — 답변 신뢰도 강화 | 인라인 인용 수, Clarify 분류 정확도, Graceful refusal |

두 개선은 서로 다른 단계에 독립적으로 작용하며, 누적 적용되어 **검색 정확도 + 답변 신뢰도**가 함께 향상됨이 수치로 확인됨.

- Reranker가 올려놓은 핵심 청크 순위(Q2 p.252 1위, Q3 p.2 1위)는 프롬프트 최적화 이후에도 **그대로 유지**
- 프롬프트 최적화로 추가된 인라인 인용(총 48개), Clarify 정확 분류(3/3), Graceful refusal(3/3)은 Reranker 단독으로는 달성 불가능한 **답변 품질 지표**
